...
class Reply(BaseFunc):
    ...
    def others_statement(self, wechat, room_wxid, from_wxid, at_user_list, msg, bf):
        ...
        #     self.send_textmsg(wechat, room_wxid, from_wxid, res, res)
            ...
            res = 'hrj：默认防守\n主A。副狙（拿到狙后与主B交换位置）\na门抢下后留守a门。防守位可以在a门后、蓝箱后、大坑、大坑上、电梯位。在中门或b区交火后前压清点。回防建议：回防b' \
                  '区时直下中门与警家队友配合解放沙地后前往b1/b2与b门队友同步回防（交流tips：到了b2怕被狙位或大箱的人打可以喊b门外队友丢闪拿枪位）回防a区时保住a门进行简单反清a' \
                  '门。等待小道回防队友同步。若小道交火则拉出补枪。\na门没抢下或选择保小道（若身位不好双a时）与队友配合死守小道（若身位不好三a时）在忍者位或L' \
                  '位打发现或退守小道或警家（保命）等待回防。如有需要帮助小道队友丢闪返清。回防建议：回防b区时应带一眼a大保证后路安全。回防a区时与队友配合清完中路或等待队友闪光回防包点。 '
            self.send_textmsg(wechat, room_wxid, from_wxid, res, res)
        elif msg == 'dust2sl':
            res = 'sl：默认防守\n副A。自由交火权\na门抢下后根据信息（如小道快上落位包点等待队友反清信号或闪光）快速回中门在根据对手喜好（如喜欢打a则落位警家方向、喜欢打b' \
                  '则落位沙地方向）进行防守打发现。若落位警家方向被夹b烟糊住则拿道具拖慢进攻等待a门队友清中路。若a门告破与b区队友清干净中路后进行回防。（交流tips：若落位b门外沙地对手进行夹b则可以在b' \
                  '门外箱后或狗洞外木架问队友要闪背闪并反清）若有需要向b点丢道具配合队友。\na门没抢下或选择保小道与队友配合死守小道。架点小套路：1与队友在小道台阶附近双架，队友架住b1自己架住小道。2' \
                  '队友直架小道在小道箱前进行补枪形成交叉火力。3队友台阶上架第二枪位死躲台阶下等待队友枪声信号。开枪就拉形成上下联动枪位。回防建议：回防a区应与队友清空中路小道后回防。回防b' \
                  '区与队友清空中路以及沙地后回防。 '
            self.send_textmsg(wechat, room_wxid, from_wxid, res, res)
        elif msg == 'dust2xf':
            res = 'xf：默认防守\n主B。（副狙拿到狙后与副B交换位置）\n过中门时跳看一眼暗道取得信息。若有人在暗道里跑大概率快上小道告知队友。防rua道具丢完后若a门抢下死守b区。防守位可以在b' \
                  '包点、假门、狙位、大箱。根据队友站位和道具量选择直架或不直架（若没有道具选择听声音架第二枪位）与队友进行交流不得卖队友。a门没抢下或告破则与副b队友进行前压b区清理b2/b1' \
                  '与中路队友汇合进行回防。放a门保小道则死守b区根据队友指挥进行回防。回防建议：一定要与队友配合清完中路后回防、若得到信息如a门敌方来的很慢或战线拉的很长并没有a门外控制权则可从b2从匪家绕路从a' \
                  '门进行回防。注意️快速。若没有得到信息从b1回到中路与队友汇合选择小道警家或a大进行回防。 '
            self.send_textmsg(wechat, room_wxid, from_wxid, res, res)
        elif msg == 'dust2cth':
            res = 'cth：默认防守\n副B。（副狙拿到狙后与主A交换位置）\n过中门时跳看一眼暗道取得信息。若有人在暗道里跑大概率快上小道告知队友。在主b将道具交与防rua后将自身伤害型道具与烟雾弹交与主b' \
                  '（若敌方选择rushB则不用）a门抢下在b门附近晃身看中路信息。敌人若夹b：a区队友没有及时回防时则退守车位死角与主b队友交流别漏给b门或狗洞。a' \
                  '区队友及时回防到沙地帮助队友丢闪清理中路。若中路占下则根据主b队友站位（如假门或大箱则在白车、如包点或狙位则在车位死角）选择点位。听b1/b2信息a门没抢下或告破与队友配合清理b2/b1' \
                  '完成后于b1进行回防。放a门保小道与a门抢下时不变。 '
            self.send_textmsg(wechat, room_wxid, from_wxid, res, res)
        elif msg == 'dust2a':
